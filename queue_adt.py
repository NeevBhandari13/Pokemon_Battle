...
class CircularQueue(Queue[T]):
    ...
    def __str__(self) -> str:
        """Returns string detailing health and level of pokemon in the queue

        :comp: O(n) complexity where n is the length of the queue
        :returns: string with health and level of pokemon 
        """
        result = ""
        current = self.front
        for i in range(len(self)):
            result += self.array[current].__str__()
            current = (current + 1) % len(self.array)
            if i != len(self) - 1:
                result += ","
        return result
